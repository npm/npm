  ...
  mapToRegistry(dep.name || dep.rawSpec, npm.config, iferr(next, function (url, auth) {
    ...
    function pickVersionFromRegistryDocument (pkg) {
      ...
      if (dep.type === 'tag') {
        ...
        log.silly('pickVersionFromRegistryDocument', 'dep.spec', dep.spec, 'tag', npm.config.get('tag'), 'versions', versions)
        var latestVersion = pkg['dist-tags'][npm.config.get('tag')]
        // Is latest, or the requested tag, satisfactory?
        if (!(latestVersion && semver.satisfies(latestVersion, dep.spec, true))) {
          // If not, return the most recent version that matches our spec
          latestVersion = semver.maxSatisfying(versions, dep.spec, true)
        }
        // if maxSatisfying came up empty, default to the tagged version
        // FIXME: this is different from lib/cache/add-named.js's logic,
        // which errors, which is a strong single that this logic should
        // be factored out into an independent function
        if (!latestVersion) latestVersion = pkg['dist-tags'][npm.config.get('tag')]
