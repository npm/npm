  ...
  mapToRegistry(dep.name || dep.rawSpec, npm.config, iferr(next, function (url, auth) {
    ...
    function pickVersionFromRegistryDocument (pkg) {
      ...
      if (dep.type === 'tag') {
        ...
        // Is latest satisfactory?
        if (semver.satisfies(latestVersion, dep.spec, true)) {
          return returnAndAddMetadata(pkg.versions[latestVersion])
        }

        // Failing that, return the most recent version that matches our spec
        var ms = semver.maxSatisfying(versions, dep.spec, true)
        if (ms) return returnAndAddMetadata(pkg.versions[ms])
      }
