...
function addBundled (pkg, next) {
  ...
  if (requested.type === 'directory') {
    pkg._bundled = null
    return readPackageTree(pkg._requested.fetchSpec, null, npm.config.get('package-json-file'), function (er, tree) {
  ...
  pacote.extract(pkg._resolved || pkg._requested || npa.resolve(pkg.name, pkg.version), target, opts).then(() => {
    ...
    readPackageTree(target, null, npm.config.get('package-json-file'), (err, tree) => {
