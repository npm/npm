...
function logAndFinish(tracker, spec, cb) {
  return function (er) {
  }
}
...
module.exports.addShrinkwrap = function addShrinkwrap (pkg, next) {
  if (pkg._shrinkwrap) return next(err, pkg)
  if (!pkg._inCache) {
    cache.add(pkg._spec, null, pkg._where, false, iferr(next, function (cachedPkg) {
      pkg._inCache = true
...
module.exports.addBundled = function addBundled (pkg, next) {
  ...
  getCacheStat(iferr(next, function (cs) {
    ...
    unpack(tarball, target, null, null, cs.uid, cs.gid, iferr(next, function (data) {
