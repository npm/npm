...
function replicateMany(fromRegistry, toRegistry, what, where, family, ancestors, explicit, cb) {
	...
	function done(er) {
		...
		// This function serializes getting/publishing of the targets
		if (deps.length === 0) {
			// No dependencies left over
			cb();
		} else {
			var next = deps.pop();
			var i, l = published.length;
			for (i = 0; i < l; i++) {
				if (published[i] === next) {
					// Have something left?
					if (deps.length > 0) {
						// Get the next target
						next = deps.pop();
						console.log("already published " + published[i] + ", checking " + next);
						// Start over in our array
						i = 0;
					} else {
						// We're done, nothing to do anymore
						console.log("already published " + published[i]);
						i = l;
					}
				}
			}
...
function publish(fromRegistry, toRegistry, target, family, ancestors, cb_) {
	...
	var pkg = path.join(npm.cache, target.name, target.version, "package.tgz");
	console.log("publish - pkg:", pkg);

	var i, l = published.length;
	for (i = 0; i < l; i++) {
		if (published[i] === target._id) {
			console.log("already published " + target._id);
			cb_();
			return;
		}
	}
	...
	npm.commands.publish([pkg], function (er) {
		published.push(target._id);
		if (er && er.message == "publish fail") {
			console.info("publish failed, assuming " + target._id + " is already in registry");
			er = null;
		}
		cb_(er);
	});
