  ...
  asyncMap(differences, function (action, done) {
    ...
    if (pkg.isAsset) {
      switch (cmd) {
        case 'add':
        case 'update':
          addAssetSteps(decomposed, pkg, done)
          break
        case 'move':
          removeAssetSteps(decomposed, pkg.oldPkg, () => {
            addAssetSteps(decomposed, pkg, done)
          })
          break
        case 'remove':
          removeAssetSteps(decomposed, pkg, done)
          break
        default:
...
function addAssetSteps (decomposed, pkg, done) {
  decomposed.push(['write-asset-index', pkg])
  decomposed.push(['rewrite-asset-js', pkg])
  return addSteps(decomposed, pkg, done)
}
...
function removeAssetSteps (decomposed, pkg, done) {
  decomposed.push(['remove-asset-index', pkg])
  return removeSteps(decomposed, pkg, done)
}
