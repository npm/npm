...
var inflateShrinkwrap = safe.recurseLimitSync(100, 1000, function (topPath, tree, swdeps, finishInflating, $$recurse$$) {
  ...
  function andInflate (name, next) {
    return function (requested) {
      ...
      if (childIsEquivalent(sw, requested, child)) {
        ...
        return $$recurse$$(topPath, child, dependencies || {}, next)
      } else {
  ...
  function andAddChild (pkg, dependencies, next) {
    return function () {
      ...
      $$recurse$$(topPath, child, dependencies || {}, next)
    }
  }
}
