...
actions['write-asset-index'] = require('./action/write-asset-index.js')
actions['rewrite-asset-js'] = require('./action/rewrite-asset-js.js')
actions['remove-asset-index'] = require('./action/remove-asset-index.js')
...
function doSerial (type, staging, actionsToRun, log, next) {
  validate('SSAOF', arguments)
  const acts = actionsToRun.filter((todo) => todo[0] === type)
  log.silly('doSerial', '%s %d', type, acts.length)
  runSerial(type, staging, acts, log, next)
}

exports.doReverseSerial = doReverseSerial
function doReverseSerial (type, staging, actionsToRun, log, next) {
  validate('SSAOF', arguments)
  const acts = actionsToRun.filter((todo) => todo[0] === type).reverse()
  log.silly('doReverseSerial', '%s %d', type, acts.length)
  runSerial(type, staging, acts, log, next)
}
