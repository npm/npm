...
// if the -S|--save or -D|--save-dev options are specified, then write installed
// packages as dependencies or devDependencies to a package.json file (and
// npm-shrinkwrap.json file, if applicable).
...
function saveShrinkwrap (tree, next) {
  ...
  fs.stat(saveTarget, function (er, stat) {
    ...
    var silent = false
    createShrinkwrap(tree.path, tree.package, hasDevOnlyDeps, silent, next)
  })
}
