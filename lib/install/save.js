...
const configGet = require('../config/get.js')

// if the -S|--save option is specified, then write installed packages
// as dependencies to a package.json file.

exports.saveRequested = function (tree, opts, andReturn) {
  validate('OOF', arguments)
  savePackageJson(tree, opts, andWarnErrors(andSaveShrinkwrap(tree, opts, andReturn)))
}

function andSaveShrinkwrap (tree, opts, andReturn) {
  validate('OOF', arguments)
  return function (er) {
    validate('E', arguments)
    saveShrinkwrap(tree, opts, andWarnErrors(andReturn))
  }
}
...
function saveShrinkwrap (tree, opts, next) {
  validate('OOF', arguments)
...
function savePackageJson (tree, opts, next) {
  validate('OOF', arguments)
  const saveBundle = configGet(opts, 'save-bundle')
...
exports.getSaveType = function (tree, arg, opts) {
  validate('O|OO|OOO', arguments)
  if (arguments.length === 1) {
    opts = tree
    tree = null
  }
  var globalInstall = configGet(opts, 'global')
  var noSaveFlags = !configGet(opts, 'save') &&
                    !configGet(opts, 'save-dev') &&
                    !configGet(opts, 'save-prod') &&
                    !configGet(opts, 'save-optional') &&
                    !configGet(opts, 'save-bundle') &&
                    !configGet(opts, 'save-assets')
  if (globalInstall || noSaveFlags) return null

  if (configGet(opts, 'save-assets')) {
    return 'assetDependencies'
  } else if (configGet(opts, 'save-optional')) {
    return 'optionalDependencies'
  } else if (configGet(opts, 'save-dev')) {
    return 'devDependencies'
  } else if (configGet(opts, 'save-prod')) {
    return 'dependencies'
  } else {
    if (arg) {
