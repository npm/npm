...
module.exports.asReadInstalled = function (tree, filter) {
  mutateIntoLogicalTree(tree)
  return translateTree(tree, filter)
}

function translateTree (tree, filter) {
  return translateTree_(tree, filter, new Set())
}

function translateTree_ (tree, filter, seen) {
  ...
  tree.children.forEach(function (child) {
    if ((filter.skipAssets && child.isAsset) || (filter.skipDeps && !child.isAsset)) return
    const dep = pkg.dependencies[moduleName(child)] = translateTree_(child, filter, seen)
    if (child.fakeChild) {
      dep.missing = true
  ...
  if (!filter.skipAssets) {
    Object.keys(tree.missingAssets).forEach(function (name) {
      markMissing(name, tree.missingAssets[name])
    })
  }
