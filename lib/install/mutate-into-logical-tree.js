...
var computeVersionSpec = require('./deps.js').computeVersionSpec
...
function translateTree_ (tree, seen) {
  ...
  pkg.path = tree.path
  if (tree.requiredBy.length) {
    const parent = tree.requiredBy[0]
    const deps = parent.package._dependencies || parent.package.dependencies || {}
    const devDeps = parent.package.devDependencies || {}
    const req = deps[pkg.name] || devDeps[pkg.name]
    const parsed = npa.resolve(pkg.name, req)
    if (!parsed.registry) {
      pkg.version = parsed.toString()
    }
  }
  ...
  if (tree.isLink) pkg.link = tree.realpath
  return pkg
}
