...
function moveModuleOnly (from, to, log, done) {
  ...
  mv(toModules, tempToModules, removeDestination(done))
  ...
  function moveToModulesBack (next) {
    return function () {
      ...
      mv(tempToModules, toModules, iferr(done, next))
    }
  }
  ...
  function moveNodeModules (next) {
    return function () {
      ...
      mv(fromModules, tempFromModules, iferr(doMove(next), doMove(moveNodeModulesBack(next))))
    }
  }
  ...
  function moveNodeModulesBack (next) {
    return function () {
      mkdirp(from, iferr(done, function () {
        ...
        mv(tempFromModules, fromModules, iferr(done, next))
      }))
    }
  }
}
