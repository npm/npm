...
module.exports = function (staging, pkg, log) {
  ...
  if (requested.type === 'directory') {
    ...
    return makeParentPath(pkg.path)
      ...
      .catch((ex) => {
        return gentlyRm(pkg.path).then(() => symlink(relative, pkg.path, 'junction'))
      })
  } else {
    return makeParentPath(pkg.realpath)
      ...
      .catch((err) => {
        if (movedDestAway) {
          return gentlyRm(pkg.realpath).then(moveOldDestinationBack).then(() => {
            throw err
          })
        } else {
          throw err
