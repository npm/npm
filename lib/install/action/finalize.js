...
var fsMove = require('../../utils/fs-move')
...
module.exports = function (top, buildpath, pkg, log, next) {
  ...
  function destStated (doesNotExist) {
    if (doesNotExist) {
      fsMove(buildpath, pkg.path, whenMoved)
    } else {
      moveAway()
    }
  }
  ...
  function moveAway () {
    fsMove(pkg.path, delpath, whenOldMovedAway)
  }

  function whenOldMovedAway (renameEr) {
    if (renameEr) return next(renameEr)
    fsMove(buildpath, pkg.path, whenConflictMoved)
  }
  ...
  function whenConflictMoved (renameEr) {
    ...
    if (renameEr) return fsMove(delpath, pkg.path, function () { next(renameEr) })
  ...
  function moveModules (mkdirEr, files) {
    ...
    asyncMap(files, function (file, done) {
      ...
      fs.stat(to, function (er, info) {
        if (er) return fsMove(from, to, done)
