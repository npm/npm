...
module.exports = function (top, buildpath, pkg, log, next) {
  ...
  cache.unpack(
    pkg.package.name, pkg.package.version, buildpath, null, null, user, group,
...
function andStageBundledChildren (pkg, buildpath, log, next) {
  // Dependencies bundled inside a scoped module need an extra step up when
  // resolving the path to the staging directory.
  var staging = buildpath
  do {
    staging = path.resolve(staging, '..')
  } while (staging.length > 1 && path.basename(staging) !== '.staging')
  if (staging.length <= 1) {
    throw new Error('???')
  }
...
function stageBundledModule (bundler, child, staging, parentPath, next) {
  ...
  asyncMap(
    child.children,
    andStageBundledModule(bundler, staging, stageFrom),
    iferr(next, moveModule)
  )
