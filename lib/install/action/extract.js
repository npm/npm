...
const ENABLE_WORKERS = false && process.platform === 'darwin'
...
function extract (staging, pkg, log) {
  ...
  let strip
  let filter
// this is the least effecient path
// better would be to both filter out unknown deps in the filter step
// AND rewrite extract paths in onentry
  if (pkg.fromBundle) {
    let bundler = pkg.fromBundle.path
    if (bundler) {
      srcPkg = pkg.fromBundle
    } else {
      while (srcPkg.fromBundle) {
        srcPkg = srcPkg.parent
      }
      bundler = srcPkg.path
    }
    const location = path.relative(bundler, pkg.path)
    const locationBits = location.split(/[/\\]/)
    strip = locationBits.length + 1
    log.error('extract', pkg.package.name, 'is bundled in', bundler, 'going to', location, 'strip', strip)
    const matchName = new RegExp('^[^/]+[/]' + location)
    filter = (name, entry) => {
      return matchName.test(name)
    }
  } else {
    filter = (name, entry) => {
  ...
  const opts = pacoteOpts({
    integrity: srcPkg.package._integrity
  })
  if (strip) opts.strip = strip
  if (filter) opts.filter = filter
  const args = [
    srcPkg.package._resolved
    ? npa.resolve(srcPkg.package.name, srcPkg.package._resolved)
    : srcPkg.package._requested,
    extractTo,
    opts
  ]
