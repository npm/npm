...
function stageBundledModule (bundler, child, staging, parentPath, next) {
  ...
  // if this module bundles its own deps and it's not coming from OUR bundle then
  // we're not responsible for staging its children.
  if (!child.fromBundle && child.package.bundleDependencies) return finishModule()
