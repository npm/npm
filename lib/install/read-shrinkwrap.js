...
function parsePkgLock (str, filename) {
  ...
  try {
    ...
    if (isDiff(str)) {
      log.warn('conflict', `A git conflict was detected in ${filename}.`)
      log.warn('conflict', 'Attempting to auto-resolve.')
      log.warn('conflict', 'To make this happen automatically on')
      log.warn('conflict', 'git rebase/merge, use npm-merge-driver:')
