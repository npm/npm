...
function loadDeps (tree, log, next) {
  ...
  if (tree.path !== tree.realpath) return andFinishTracker.now(log, next)
  tree.loaded = true
...
exports.loadDevDeps = function (tree, log, next) {
  ...
  asyncMap(Object.keys(tree.package.devDependencies), function (dep, done) {
    ...
    addDependency(spec, tree, logGroup, done)
  }, andForEachChild(loadDeps, andFinishTracker(log, next)))
}
...
function addDependency (spec, tree, log, done) {
  ...
  var next = andAddParentToErrors(tree, cb)
  realizePackageSpecifier(spec, tree.path, function (er, req) {
    var child = findRequirement(tree, req.name, npa(spec))
    if (child) {
      resolveWithExistingModule(child, child.package, tree, log, next)
    }
    else {
      fetchPackageMetadata(req, tree.path, log.newItem('fetchMetadata'), iferr(next, function (pkg) {
        resolveRequirement(pkg, tree, log, next)
      }))
    }
  })
