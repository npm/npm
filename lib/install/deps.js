...
exports.loadDevAndProdDeps = function (tree, log, next) {
  validate('OOF', arguments)

  if (!tree.package.devDependencies && !tree.package.dependencies) return andFinishTracker.now(log, next)

  var devShrinkwrapped = tree.children.some(function (child) { return child.shrinkwrapDev })

  var deps = devShrinkwrapped ? {} : (tree.package.dependencies || {})
  var devDeps = tree.package.devDependencies || {}

  var depKeys = Object.keys(deps).filter(function (d) {
    return !(tree.package.optionalDependencies &&
      tree.package.optionalDependencies[d] &&
      !npm.config.get('optional'))
  })
  ...
  for (var devKey in devDeps) {
    all[devKey] = devDeps[devKey]
  }
  for (var prodKey in finalDeps) {
    all[prodKey] = finalDeps[prodKey]
  }

  asyncMap(Object.keys(all), function (dep, done) {
    var version = all[dep]
    addDependency(dep, version, tree, log.newGroup('loadDevAndProdDeps:' + dep), andHandleOptionalErrors(log, tree, dep, done))
  }, andForEachChild(loadDeps, andFinishTracker(log, next)))
}
