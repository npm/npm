...
function isDep (tree, child, cb) {
  if (child.fromShrinkwrap) return process.nextTick(function () { cb(true) })
  var name = moduleName(child)
  isProdDep(tree, name, function (prodSpec) {
    var matches
    if (prodSpec) matches = doesChildVersionMatch(child, prodSpec, tree)
    if (matches) return cb(true, prodSpec)
    isDevDep(tree, name, function (devSpec) {
      if (!devSpec) return cb()
      cb(doesChildVersionMatch(child, devSpec, tree), prodSpec, devSpec)
    })
  })
}
...
function isDevDep (tree, name, cb) {
  ...
  if (reqVer == null) return cb()
  childDependencySpecifier(tree, name, reqVer, function (er, spec) {
    cb(spec)
  })
}
...
function addRequiredDep (tree, child, cb) {
  isDep(tree, child, function (childIsDep, childIsProdDep) {
