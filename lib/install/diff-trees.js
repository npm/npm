...
module.exports = function (oldTree, newTree, differences, opts, log, next) {
  validate('OOAOOF', arguments)
  pushAll(differences, sortActions(diffTrees(oldTree, newTree, opts)))
  log.finish()
  next()
}
...
var diffTrees = module.exports._diffTrees = function (oldTree, newTree, opts) {
  validate('OO|OOO', arguments)
  var differences = []
  ...
  const alsoDev = /^dev(elopment)?$/.test(configGet(opts, 'also'))
  const onlyAssets = /^assets$/.test(only)

  const includeDev = configGet(opts, 'dev') ||
    (!onlyProd && !configGet(opts, 'production')) ||
    onlyDev || alsoDev
  const includeProd = !onlyDev && !onlyAssets
  const includeAsset = configGet(opts, 'assets') && !onlyProd && !onlyDev
