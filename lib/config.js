...
function edit (cb) {
  if (!process.env.EDITOR) return cb(new Error("No EDITOR environment var set set."))
  var f = ini.get(ini.get("global") ? "globalconfig" : "userconfig")
  exec("sh", ["-c", process.env.EDITOR + " "+f], cb)
}
