...
function shrinkwrapDeps (dev, problems, deps, tree) {
  ...
  Object.keys(tree.missingDeps).forEach(function (name) {
    ...
    if (invalid) {
      problems.push('invalid: ' + invalid.package._id + invalid.path)
    } else {
      var topname = tree.package._id ||
                    (tree.package.name && tree.package.name + tree.package.version ? '@' + tree.package.version : '')
