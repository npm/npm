...
function shrinkwrap_ (pkginfo, silent, dev, cb) {
  if (!dev) {
    ...
    readJson(path.resolve(npm.prefix, "package.json"), function (er, data) {
      ...
      pkginfo.problems = Object.keys(pkginfo.dependencies).reduce(function (seed, key) {
        var val = pkginfo.dependencies[key];
        if (!val.problems) return seed;
        seed = [].concat.apply(val.problems, seed);
        return seed;
      }, null);
      if (pkginfo.problems) {
