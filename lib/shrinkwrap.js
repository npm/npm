...
const doesChildVersionMatch = require('./install/does-child-version-match.js')
  ...
  sortModules(tree.children).forEach(function (child) {
    ...
    if (child.requires.length) {
      ...
      sortModules(child.requires).forEach((required) => {
        const name = moduleName(required)
        const deps = child.package.dependencies || {}
        const devDeps = child.package.devDependencies || {}
        const optDeps = child.package.optionalDependencies || {}
        const spec = deps[name] || optDeps[name] || devDeps[name]
        // don't switch old lock files from storing version to storing ranges all at once…
        const oldSpec = child.swRequires && child.swRequires[name] && npa.resolve(name, child.swRequires[name])
        if (oldSpec && oldSpec.type === 'version' && doesChildVersionMatch(required, oldSpec)) {
          pkginfo.requires[name] = child.swRequires[name]
        } else {
          pkginfo.requires[name] = spec
        }
      })
    }
