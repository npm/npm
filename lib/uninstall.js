...
function uninstall (args, cb) {
  ...
  const packageLockOnly = !!npm.config.get('package-lock-only')
  ...
  if (args.length) {
    new Uninstaller(where, dryrun, packageLockOnly, args).run(cb)
  } else {
    ...
    readJson(path.resolve(npm.localPrefix, 'package.json'), function (er, pkg) {
      ...
      new Uninstaller(where, dryrun, packageLockOnly, [pkg.name]).run(cb)
    })
  }
}
