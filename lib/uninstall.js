...
function uninstall (args, cb) {
  ...
  unpackArgs(args, function (er, args) {
    if (args.length) log(args, "removing")
    if (er) return cb(er)
  ...
  asyncMap(args, function (pv, cb) {
    getDependents(pv, function (er, deps) {
      ...
      asyncMap(deps, function (dep, cb) {
        ...
        readJson( path.join(npm.dir, depName, depVer, "package", "package.json")
                , function (er, data) {
        ...
        if (depNames.length) {
          if (force) {
            ...
            log.warn(depNames, "forced uninstall")
          } else return cb(new Error(pkg+" depended upon by "+depNames.join(",")+".\n"
                                    +"Cannot remove"))
        }
        readJson(path.join(npm.dir, pv[0], pv[1], "package","package.json"), cb)
      })
    })
  }, cb)
}
    ...
    fs.readdir(path.join(npm.dir, pkg), function (er, vers) {
      ...
      if (!recursive) cb(null, pvs)
        ...
        if (deps.length) {
          ...
          log.warn(deps, "recursive uninstall")
