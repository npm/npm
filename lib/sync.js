...
function sync(args, silent, cb) {
  ...
  readJson(jsonFile, function(error, config) {
    ...
    if (0 == syncPackagesCount) {
      for (var key in packageConfig.dependencies) allPackages.push(key)
      for (var key in packageConfig.optionalDependencies) allPackages.push(key)
      for (var key in packageConfig.devDependencies) allPackages.push(key)
    }
