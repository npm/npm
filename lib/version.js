...
function _commit (version, localData, cb) {
  ...
  var signTag = npm.config.get('sign-git-tag')
  var tagFlag = signTag ? '-sm' : '-am'
  var signCommit = npm.config.get('sign-git-commit')
  chain(
    [
      git.chainableExec([ 'add', packagePath ], options),
      localData.hasShrinkwrap && git.chainableExec([ 'add', 'npm-shrinkwrap.json' ], options),
      signCommit
        ? git.chainableExec([ 'commit', '-S', '-m', message ], options)
