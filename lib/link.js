...
function save (pkgs, cb) {
  var deps = getSaveType(pkgs)
  if (!deps) return cb()

  var where = path.resolve(npm.dir, '..')
  var argPaths = pkgs.map(function (arg) {
    return path.resolve(where, arg)
  })

  readPackageTree(where, iferr(cb, function (currentTree) {
    var idealTree = currentTree
    idealTree.children = idealTree.children.map(function (child) {
      if (includes(argPaths, child.realpath)) {
        child.save = deps
      }
      return child
    })
    saveRequested(pkgs, idealTree, cb)
  }))
}
