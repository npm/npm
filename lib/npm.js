...
function load (npm, cli, cb) {
  which(process.argv[0], function (er, node) {
    ...
    npmconf.load(cli, builtin, function (er, config) {
      ...
      var progressEnabled
      switch (npm.config.get('progress')) {
        case "always": progressEnabled = true; break
        case false: progressEnabled = false; break
        // default this to color settings, because logic should be the same;
        // if user sets --no-color, he usually doesn't want ANY ansi sequences 
        default: progressEnabled = npm.color;
      }
      npm.progress = require('yapm-progress')
                       .enable(progressEnabled)
                       // whenever somebody writes to process.std(out|err),
                       // we should clear the line and update progressbar after
                       .intercept(process.stdout)
                       .intercept(process.stderr)
                       .output(npm.config.get("logstream"))
