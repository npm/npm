...
function request (method, where, what, cb) {
  ...
  if (what instanceof File) {
    var b = new Buffer(1024 * 2)
      , sent = 0
      , i = 0
      , pct = 0
    // FIXME: This timeout hackery is working around an interesting bug
    // in node.  Large request bodies can flood the connection, making
    // things behave in odd ways.  Come back here and remove this when
    // node's http client stops being weird with big messages.
    fsBinding.read(what.fd, b, 0, b.length, null, function W (er, bytesRead) {
      if (er) return log.er(cb, "Failure to read tarball")(er)

      pct = Math.round(sent / what.length * 100 )
      if (!bytesRead || pct > i) {
        log(pct+"%", "uploading")
        i = pct + 7
      }
      ...
      if (!bytesRead) {
        request.end()
        return log("uploaded", "registry")
      }
      request.write(b)
      setTimeout(function () {
        fsBinding.read(what.fd, b, 0, b.length, null, W)
      }, 100)
    })
    return
