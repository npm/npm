...
function pack (targetTarball, folder, pkg, dfc, cb) {
  ...
  makeList(folder, pkg, true, dfc, function (er, files) {
    if (er) return cb(er)
...
function makeList (dir, pkg, excludes, dfc, cb) {
  ...
  makeList_(dir, pkg, excludes, dfc, function (er, files) {
    if (er) return cb(er)
...
function cleanupResolveLinkDep (dir, cb) {
  ...
  delete _linkDepReg[dir]
  if (!d) return cb()

  asyncMap(d, function (d, cb) {
    rm(d[1], function (er) {
      if (er) return cb(er)
      fs.symlink(d[0], d[1], function (er) { cb(er, d[2]) })
    })
  }, function (er, also) {
    // also clean up the other ones that were perhaps also messed up
    // by the cache resolution bs stuff.
    if (er) return cb(er)
    asyncMap(also, cleanupResolveLinkDep, cb)
  })
}
...
function resolveLinkDep (dir, file, resolved, target, pkg, cb) {
  ...
  var reg = _linkDepReg[pkg.path] = _linkDepReg[pkg.path] || []
    , f = path.resolve(dir, file)
    , entry = [target, f, resolved]
  reg.push(entry)
  ...
  rm(f, function (er) {
    ...
    cache.add(resolved, function (er, data) {
      ...
      cache.unpack(data.name, data.version, f, function (er, data) {
        if (er) return cb(er)
        // now clear out the cache entry
        cache.clean([data._id], function (er) { cb(er, file) })
      })
    })
  })
}
