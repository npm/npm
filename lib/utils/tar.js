...
function packFiles (targetTarball, parent, files, pkg, cb) {
  ...
  log.warn(targetTarball, "tarball")
  log.warn(files, "files")
  fstream.Reader({ type: "Directory"
                 , path: parent
                 //, filter: function () {
                 //    log.warn([files.indexOf(this.path), this.path], "reader filter")
    ...
    .on("entry", function E (entry) {
      log.warn(entry.path, "add entry")
      entry.on("entry", E)
    })
    .pipe(tar.Pack({}))
    // .on("data", function (c) {
    //   log.warn(c.length, "tar data")
    // })
  ...
  ;(function ADD (f) {
    if (!f) pack.end()
    log.warn(path.resolve(parent, f), "add to tarball")
    f = fstream.Reader({ type: "File", path: path.resolve(parent, f) })
    pack.add(f)
    f.on("close", function () { ADD(files.shift()) })
  })(files.shift())
}
