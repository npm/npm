...
function lifecycle_ (pkg, stage, wd, env, unsafe, failOk, cb) {
  ...
  if (packageLifecycle) {
	...
	var projNPMrc = null
	if (npm.config.sources.project && npm.config.sources.project.data) {
		projNPMrc = npm.config.sources.project.data;
	}
    if (stage === "install" && projNPMrc && projNPMrc.nw_version) {
	  var copy = pkg.scripts[stage]
	  if (copy.indexOf('node-gyp') > -1) {
		env.npm_lifecycle_script = copy.replace(/node-gyp/g, "nw-gyp --target="+projNPMrc.nw_version+" --arch="+process.arch)
	  }
	  if (copy.indexOf('node-pre-gyp') > -1) {
		env.npm_lifecycle_script = copy.replace(/node-pre-gyp/g, "node-pre-gyp --runtime=node-webkit --target="+projNPMrc.nw_version+" --arch="+process.arch)
	  }
	}
  }
