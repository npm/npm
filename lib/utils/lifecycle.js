...
function lifecycle (pkg, stage, wd, unsafe, failOk, cb) {
  ...
  validWd(wd || path.resolve(npm.dir, pkg.name), function (er, wd) {
    ...
    log.silly('lifecycle[' + stage + ']', 'valid working directory:', wd)
...
function lifecycle_ (pkg, stage, wd, env, unsafe, failOk, cb) {
  ...
  log.silly('lifecycle[' + stage + ']', 'PATH[' + env[PATH].length + ']=' + env[PATH])
  var packageLifecycle = pkg.scripts && pkg.scripts.hasOwnProperty(stage)

  if (packageLifecycle) {
    // define this here so it's available to all scripts.
    log.silly('lifecycle[' + stage + ']', 'lifecycle script for', stage, ':', pkg.scripts[stage])
    env.npm_lifecycle_script = pkg.scripts[stage]
  } else {
    log.silly('lifecycle[' + stage + ']', 'no script for', stage, ', continuing')
  }
...
function runCmd_ (cmd, pkg, env, wd, stage, unsafe, uid, gid, cb_) {
  ...
  log.silly('lifecycle[' + stage + ']', 'ENV:     ', env)
  log.silly('lifecycle[' + stage + ']', 'CWD:     ', wd)
  log.silly('lifecycle[' + stage + ']', 'ARGS:    ', [shFlag, cmd])
  log.silly('lifecycle[' + stage + ']', 'SPAWN:   ', cmd)
  ...
  proc.on('close', function (code, signal) {
    log.silly('lifecycle[' + stage + ']', 'RETURNED: code=', code, ' signal=', signal)
    if (signal) {
