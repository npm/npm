...
function runCmd_ (cmd, pkg, env, wd, stage, unsafe, uid, gid, cb_) {
  ...
  if (process.platform === 'win32') {
    ...
    // check if we have a composed command
    // e.g.: `"test": "node ../test/test"`
    if (cmd.indexOf(' ') !== -1) {
      // split the command
      var cmdChunks = cmd.split(' ')

      cmd = cmdChunks.map(function (chunk) {
        // let's assume that a path starts with either `./` or `../`
        // maybe using `lstat` or `lstatSync` could be cleaner
        if (chunk.indexOf('./') !== -1) {
          // return the normalized path
          return path.normalize(chunk)
        }
        // return unchanged value
        return chunk
      }).join(' ')
    } else {
      // assume that we have only a path
      // e.g.: `"test": "../node_modules/test"`
