...
function lifecycle_ (pkg, stage, wd, env, unsafe, failOk, cb) {
  ...
  p.forEach(function (pp) {
    alterPath.unshift(path.join(acc, 'node_modules', '.bin'))
  ...
  alterPath.unshift(path.join(__dirname, '..', '..', 'bin', 'node-gyp-bin'))
...
function runCmd_ (cmd, pkg, env, wd, stage, unsafe, uid, gid, cb_) {
  ...
  log.verbose('lifecycle', logid(pkg, stage), 'PATH:', env[getPathVar(env, process.platform)])
