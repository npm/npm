...
function encryptAuth (config) {
  ...
  var unpw = config.auth.split(":")
  unpw[0] = config.username || unpw[0]
  unpw[1] = config.password || unpw[1]
  auth = unpw.join(":")
  var key = getKey()
...
function decryptAuth (config) {
  ...
  config.username = config.username || unpw[0]
  config.password = config.password || unpw[1]
}
