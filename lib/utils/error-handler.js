...
process.on('exit', function (code) {
  ...
  if (itWorked) {
    ...
    if (!cbCalled) {
      ...
      writeLogFile()
      console.error('')
      console.error('This is an error with npm itself. Please report this error at:')
      console.error('    <https://github.com/npm/npm/issues>')
    }
    ...
    if (wroteLogFile) {
      ...
      console.error('A complete log of this run can be found in:')
...
function errorHandler (er) {
  ...
  msg.summary.concat(msg.detail).forEach(function (errline) {
    console.error.apply(log, errline.slice(1))
  })
