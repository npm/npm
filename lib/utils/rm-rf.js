...
function rm (p, cb_) {
  ...
  function cb (er) {
    if (er) {
      ...
      if (er.errno === constants.EACCES) {
        // ask for privs
        var sudoEnv ={SUDO_PROMPT:"[npm rm "+p+"] Password:"
                     ,SUDO_UID:process.getuid()}
        for (var i in process.env) sudoEnv[i] = process.env[i]
        log.warn("Upgrade permissions to remove "+p, "rm")
        return exec("sudo", ["rm", "-r", p]
                   ,sudoEnv
                   ,true
                   ,cb)
      }
