...
function gentlyRm (target, gently, base, cb) {
  ...
  if (prefixes.indexOf(targetPath) !== -1) {
    log.silly('gentlyRm', targetPath, "is part of npm and can't be removed")
  ...
  if (!gently) {
    log.silly('gentlyRm', "don't care about contents; nuking", targetPath)
  ...
  chain([
    ...
    function thenRemove (toRemove, removeBase) {
      ...
      log.silly('gentlyRm', options.purge ? 'Purging' : 'Vacuuming',
