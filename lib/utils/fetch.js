...
function fetchAndWrite (remote, fstr, headers, maxRedirects, redirects) {
  ...
  // if we're using http1, then don't do connection pooling.
  if (!http.globalAgent) opts.agent = false
  ...
  var proxy = npm.config.get(opts.secure ? "https-proxy" : "proxy")
  opts = proxyify(proxy, remote, opts)
  if (!opts) return cb(new Error("Bad proxy config: "+npm.config.get("proxy")))

  var http_ = opts.secure ? https : http

  http_.get(opts, function (response) {
    // handle redirects.
