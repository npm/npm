...
function fetch_ (remote, local, headers, cb) {
  ...
  // we don't have any access to current package name, so attempt to guess it from remote
  var packageName = remote.match(/\/([^/]+)-\d+\.\d+\.\d+(\-[^/]+)?\.tgz$/)
  packageName = packageName ? packageName[1] : remote.substr(remote.lastIndexOf('/')+1)
  npm.progress.add(packageName, req)
