...
function mapToRegistry (name, config, cb) {
  ...
  var registry = npm.config('registry', 'cli') /* || npm.config('registry', 'env') maybe? should we? */

  // if there's no command line registry then the name itself takes precedence
  var data = npa(name)
  if (!registry && data.scope) {
