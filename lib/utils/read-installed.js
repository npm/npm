  ...
  Object.keys(deps)
    ...
    .forEach(function (d) {
      ...
      var r = obj.parent
        , found = null
      while (r && !found) {
        //console.error(r._id, deps[d])
        // if r is a valid choice, then use that.
        if (r.name && r.name === d && semver.satisfies(r.version, deps[d])) {
          //console.error("parent satisfies")
          found = r
        } else {
          //console.error("looking in "+r.name )
          found = r.dependencies[d]
        }
        if (!found) {
          //console.error("walk up to parent of "+r.name)
          r = r.parent
          //console.error("parent = "+r.name)
          continue
        }
        ...
        log.verbose(found._id +" satisfies "+deps[d])
        obj.dependencies[d] = found
        log.verbose(r && !found, "continue?")
      }
      if (!found) {
