...
function processObject (tag, cb) { return function (er, json) {
  ;[ "overlay", "env", "context", "ctx", "vnd", "vendor" ]
  .forEach(function (vnd) {
    if (!json[vnd]) return undefined
    ;["node", "npm"].forEach(function (k) {
      if (!json[vnd][k]) return undefined
      for (var i in json[vnd][k]) json[i] = json[vnd][k][i]
    })
  })
