...
function processObject (opts, cb) { return function (er, json) {
  ...
  if (json.repository) {
    ...
    if (json.repository.type === "git"
      ...
      var msg = "Please use the 'git://github.com/user/project.git'\n"
              + "format for github repository URLs."
        , e = new Error(msg)
      // later: uncomment this
      // if (cb) return cb(e)
      // throw e
      log.error(msg, "incorrect json "+json._id)
      repo = repo.replace(/^https?:\/\/github.com/, 'git://github.com')
      if (!repo.match(/\.git$/)) {
        repo = repo
               .replace(/^(https?|git):\/\/github.com\/([^\/]+)\/([^\/]+)\/?$/
                       ,'git://github.com/$2/$3.git')
      }
    }
    if (repo.match(/github\.com\/[^\/]+\/[^\/]+\/?$/)
        && repo.match(/\.git\.git$/)) {
      log.win(repo, "Are you sure this is right?")
    }
