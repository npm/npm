...
function read (name, ver, forceBypass, cb) {
  ...
  // allow installations with just a shrinkwrap.json
  var jsonFile = path.join(npm.cache, name, ver, "package", "package.json")
  fs.exists(jsonFile, function (exists) {
    if (!exists)
      jsonFile = path.join(npm.cache, name, ver, "package", "npm-shrinkwrap.json")
  ...
  function thenUnpack () {
    tar.unpack(p, folder, null, null, uid, gid, function (er) {
      ...
      sha.get(p, function (er, shasum) {
        ...
        // allow installations with just a shrinkwrap.json
        var jsonFile = path.resolve(folder, "package.json")
        fs.exists(jsonFile, function (exists) {
          if (!exists)
            jsonFile = path.join(folder, "npm-shrinkwrap.json")
...
function addLocalDirectory (p, name, shasum, cb) {
  ...
  // allow installations with just a shrinkwrap.json
  var jsonFile = path.join(p, "package.json")
  fs.exists(jsonFile, function (exists) {
    if (!exists)
      jsonFile = path.join(p, "npm-shrinkwrap.json")

    readJson(jsonFile, strict, function (er, data) {
      er = needName(er, data)
      er = needVersion(er, data)
