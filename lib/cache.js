...
function ls (args, cb) {
  ...
  var lsPath = args.length ? path.normalize.apply(null, args) : '.'
  ls_(lsPath, npm.config.get('depth'), function (er, files) {
...
function clean (args, cb) {
  ...
  var f = args.length ? path.join(npm.cache, path.normalize.apply(null, args)) : npm.cache
  if (f === npm.cache) {
  ...
  switch (p.type) {
    ...
    case 'remote':
      ...
      mapToRegistry(p.raw, npm.config, function (err, uri, auth) {
        ...
        addRemoteTarball(p.fetchSpec, { name: p.name }, null, auth, cb)
      })
      break
    case 'git':
      ...
      break
    default:
      if (p.name) return addNamed(p.name, p.fetchSpec, null, cb)
