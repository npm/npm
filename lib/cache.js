...
function addNamed (name, x, cb) {
  x = x || npm.config.get("tag")
  return ( semver.valid(x) ? addNameVersion
         : semver.validRange(x) ? addNameRange
         : addNameTag
         )(name, x, cb)
}

function addNameTag (name, tag, cb) {
  registry.get(name, tag, function (er, data, json, response) {
    if (er) return cb(er)
    addNameVersion(name, data.version, cb)
  })
}
...
function addNameRange (name, range, cb) {
  registry.get(name, function (er, data, json, response) {
    ...
    if (!ms) {
      return cb(new Error("Not found: "+name+"@'"+range+"'\n"
...
function addTmpTarball (tgz, name, cb) {
  ...
  unpackTar(tgz, path.resolve(contents, "package"), function (er) {
