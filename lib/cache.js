...
var maybeGithub = require("./cache/maybe-github.js")
    ...
    fs.readdir(npm.cache, function (er, files) {
      ...
      asyncMap(
        files.filter(function (file) {
          return npm.config.get("force") || file !== "-"
        }).map(function (file) {
          return path.join(npm.cache, file)
        }),
        rm,
        cb
      )
...
function unpack (pkg, ver, unpackTarget, dMode, fMode, uid, gid, cb) {
  ...
  read(pkg, ver, false, function (er) {
    ...
    npm.commands.unbuild([unpackTarget], true, function (er) {
      ...
      var root = cachedPackageRoot({ name : pkg, version : ver })
      tar.unpack(
