...
const config = require('./config/figgy-config')
...
function add (args, where) {
  ...
  return finished(pacote.tarball.stream(spec, config({where})).resume())
}
...
function unpack (pkg, ver, unpackTarget, dmode, fmode, uid, gid) {
  return unbuild([unpackTarget], true).then(() => {
    const opts = config({dmode, fmode, uid, gid, offline: true})
