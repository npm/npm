...
// 5. resolveHead()
// 6. git clone cacheDir npm.tmp/rnd
// 7. git checkout <git-ref> (cwd: npm.tmp/rnd)
// 8. git submodule update --init --recursive (cwd: npm.tmp/rnd)
// 9. cd npm.tmp/rnd && git checkout <git-ref>
// 10. addLocalDirectory(npm.tmp/rnd)
...
function cloneGitRemote (p, u, co, origUrl, cb) {
  mkdir(p, function (er) {
    if (er) return cb(er)
    exec( npm.config.get("git"), ["clone", "--mirror", u, p]
        , gitEnv(), false
...
function archiveGitRemote (p, u, co, origUrl, cb) {
  ...
  exec(git, archive, env, false, p, function (er, code, stdout, stderr) {
    ...
    tmp = path.join(npm.tmp, Date.now()+"-"+Math.random())
    resolveHead()
  })
  ...
  function clone () {
    exec(git, ["clone", "-q", p, tmp], env, function(er, code, stdout, stderr) {
      stdout = (stdout + "\n" + stderr).trim()
      if (er) {
        log.error( "Failed cloning git-repo into " 
                 + tmp + " (" + u + ")", stderr )
        return cb(er)
      }
      log.verbose("git clone " + p + " " + tmp + " (" + u + ")", stdout)
      checkout()
    })
  }
  
  function checkout () {
    exec( git, ["checkout", co], env, false, tmp
        , function(er, code, stdout, stderr) {
      stdout = (stdout + "\n" + stderr).trim()
      if (er) {
