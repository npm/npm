...
function checkGitDir (p, u, co, origUrl, cb) {
  fs.stat(p, function (er, s) {
    ...
    var arg = "config --get remote.origin.url"
    var cmd = git + " " + arg
    var env = gitEnv()

    exec(cmd, {cwd: p, env: env}, function(er, stdout, stderr) {
...
function archiveGitRemote (p, u, co, origUrl, cb) {
  ...
  exec(git + " " + archive, {cwd: p, env: env}
  ...
  function resolveHead () {
    exec(git + " " + resolve, {cwd: p, env: env}
        , function (er, stdout, stderr) {
      stdout = (stdout + "\n" + stderr).trim()
      if (er) {
