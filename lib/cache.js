...
var cloneGitRemote = require("./git-cache.js").cloneGitRemote
function checkGitDir (p, u, co, origUrl, silent, cb) {
  // xxx git-module refactor
  var g = npm.config.get("git")
    , t = npm.tmp
    , c = npm.cache

  fs.stat(p, function (er, s) {
    if (er) return cloneGitRemote(p, u, co, origUrl, silent
      , c, t, g, log, gitDoneCb, cb)
    if (!s.isDirectory()) return rm(p, function (er){
      if (er) return cb(er)
      cloneGitRemote(p, u, co, origUrl, silent, c, t, g, log, gitDoneCb, cb)
    })
...
function gitDoneCb (tmp, resolved, cb) {
  // xxx git-module refactor
...
function archiveGitRemote (p, u, co, origUrl, cb) {
  ...
  require("./git-cache.js")
    .archiveGitRemote(p, u, co, origUrl, c, t, g, log, gitDoneCb, cb)
}
