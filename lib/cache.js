...
function afterAdd (cb) { return function (er, data) {
  ...
  getStat(function (er, cs) {
    ...
    // make sure that we continue if the user chose
    // to force the operation ignoring any chown
    // related exceptions.
    // since writeFileAtomic() chains the calls and
    // fails early on the call to chown() we must
    // prevent it from trying to.
    // TODO:issue warning about this
    var options = {};
    if (!npm.config.get('force')) {
      options = {chown : cs};
    }
    writeFileAtomic(pj, JSON.stringify(data), options, function (er) {
