...
var mkdir = require("mkdirp")
  ...
  , gitCache = require("npm-cache-git")
  ...
  lock(u, function (er) {
    ...
    var p = path.join(npm.config.get("cache"), "_git-remotes", v)
      , opts = { git: npm.config.get("git")
        ...
        , origUrl: origUrl
        , silent: silent }
      , checkGitDir = gitCache.checkGitDir
      , cloneGitRemote = gitCache.cloneGitRemote
      , archiveGitRemote = gitCache.archiveGitRemote

    checkGitDir(opts, log, rm, cloneGitRemote, archiveGitRemote
      , function (er, tmp, resolved) {
      if (er) return cb(er)
