...
function addNameVersion (name, ver, cb) {
  registry.get(name, ver, function (er, data, json) {
    ...
    mkdir(folder, function (er) {
      if (er) return log.er(cb, "couldn't create "+folder)(er)
      ...
      fetch(data.dist.tarball, tgz, function (er) {
        if (er) return log.er(cb,
          "failed to fetch "+data.dist.tarball+" to "+tgz)(er)
...
function addLocalTarball (p, cb) {
  ...
  if (p.indexOf(npm.cache) === 0 && path.basename(p) === "package.tgz") {
    return addPlacedTarball(p, cb)
  }
  unpackTar(p, tmp, function (er) {
    if (er) return log.er(cb, "Failed to unpack "+p+" into "+tmp)(er)
    addLocalDirectory(tmp, function (er, data) {
      if (er) return log.er(cb, "Failed to add "+tmp)(er)
...
function addPlacedTarball (p, cb) {
  ...
  rm(folder, function (e) {
    ...
    unpackTar(p, folder, function (er) {
      if (er) return log.er(cb, "Couldn't unpack "+p+" into "+folder)(er)
      readJson(path.join(folder, "package.json"), function (er, data) {
        if (er) return log.er(cb, "No package.json in "+p)(er)
        fs.writeFile( json
                    , JSON.stringify(data)
                    , log.er(cb, "Could not write to "+json)
                    )
      })
    })
  })
...
function addLocalDirectory (p, cb) {
  ...
  readJson(path.join(p, "package.json"), function (er, data) {
    if (er) return log.er(cb, "Couldn't read "+p+"/package.json")(er)
    var tgz = path.join(npm.cache, data.name, data.version, "package.tgz")
    packTar(tgz, p, function (er) {
      if (er) return log.er(cb, "Coudln't create "+tgz)(er)
      addLocalTarball(tgz, cb)
    })
  })
}
