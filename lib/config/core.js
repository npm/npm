...
Conf.prototype.addEnv = function (env) {
  ...
  Object.keys(env)
    ...
    .forEach(function (k) {
      ...
      // except for if there is :_ or :__ as
      // these are used for _password and __auth
      var p = k.toLowerCase()
              .replace(/^npm_config_/, '')
              .replace(/:_/, 'PLACEHOLDERONE')
              .replace(/:__/, 'PLACEHOLDERTWO')
              .replace(/(?!^)_/g, '-')
              .replace('PLACEHOLDERONE', ':_')
              .replace('PLACEHOLDERTWO', ':__')
      conf[p] = env[k]
    })
