...
function pacoteOpts (moreOpts) {
  ...
  npm.config.keys.forEach(function (k) {
    let authMatch = ''
    if (k[0] === '/') {
      authMatch = k.match(/(.*):(_authToken|username|_password|password|email|always-auth|_auth)$/)
    } else if (/^(_authToken|username|_password|password|email|always-auth|_auth)$/.test(k)) {
      authMatch = ['dummyValue', registryKey(opts.registry), k]
    }
    if (authMatch) {
