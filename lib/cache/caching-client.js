...
function get_ (uri, cachePath, params, cb) {
  ...
  if (!npm.config.get('network')) {
    if (data && !data.error) {
      return cb(null, data, '', {statusCode: 304})
    } else if (data && data.error) {
      return cb(data.error, data, '', {statusCode: 500})
    } else {
      var er = new Error('Networking disabled')
      er.code = 'ECONNREFUSED'
      return cb(er, data, '', {statusCode: 500})
    }
  }
