...
function addLocal (p, pkgData, cb_) {
  ...
  function cb (er, data) {
    if (er) {
      log.error('addLocal', 'Could not install %s', p.fetchSpec)
      return cb_(er)
    }
    if (data && !data._fromHosted) {
      data._from = path.relative(npm.prefix, p.fetchSpec) || '.'
      var resolved = path.relative(npm.prefix, p.fetchSpec)
  ...
  if (p.type === 'directory') {
    addLocalDirectory(p.fetchSpec, pkgData, null, cb)
  } else {
    addLocalTarball(p.fetchSpec, pkgData, null, cb)
  }
}
