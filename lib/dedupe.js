...
function dedupe (args, cb) {
  ...
  var dryrun = false
  var packageLockOnly = false
  if (npm.command.match(/^find/)) dryrun = true
  if (npm.config.get('dry-run')) dryrun = true
  if (npm.config.get('package-lock-only')) packageLockOnly = true
  if (dryrun && !npm.config.get('json')) npm.config.set('parseable', true)

  new Deduper(where, dryrun, packageLockOnly).run(cb)
}

function Deduper (where, dryrun, packageLockOnly) {
  validate('SBB', arguments)
  Installer.call(this, where, dryrun, packageLockOnly, [])
