...
function makeArchy_ (data, long, dir, depth, parent, d) {
  ...
  if (data._found === true && data._id) {
    if (npm.color) {
      ...
      if(data._size){
        var colorForSize;
        if(data._size.match('kB')){
          colorForSize = color.yellow;
        }
        else if(data._size.match('MB')){
          colorForSize = color.red;
        }
        out.label += colorForSize(' ' + '(' + data._size + ')')
      }
    } else {
