...
function ls (args, cb) {
  ...
  readPackageTree(dir, function (_, physicalTree) {
    ...
    readShrinkwrap.andInflate(physicalTree, function () {
      lsFromTree(dir, computeMetadata(physicalTree), args, {skipAssets: true}, cb)
    })
  })
}
...
var lsFromTree = ls.fromTree = function (dir, physicalTree, args, filter, cb) {
  ...
  var data = mutateIntoLogicalTree.asReadInstalled(physicalTree, filter)
