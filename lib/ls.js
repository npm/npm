...
function filterByEnv (data) {
  ...
  Object.keys(data.dependencies).forEach(function (name) {
    if (!runEnv.onlyDev && inList(devKeys, name) && data.dependencies[name].missing) {
      return
    }

    if ((runEnv.onlyDev && inList(devKeys, name)) ||          // only --dev
        (runEnv.onlyProduction && inList(prodKeys, name)) ||  // only --production
        (!runEnv.onlyDev && !runEnv.onlyProduction)) {        // no --production|--dev|--only=xxx
