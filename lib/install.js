...
Installer.prototype.addPackage = function (spec) {
...
Installer.prototype.runTopLevelLifecycles = function (cb) {
  ...
  if (!this.args.length) steps.push(
    [doOneAction, 'preinstall', staging, this.idealTree, trackLifecycle.newGroup('preinstall:.')],
    [doOneAction, 'build', staging, this.idealTree, trackLifecycle.newGroup('build:.')],
    [doOneAction, 'postinstall', staging, this.idealTree, trackLifecycle.newGroup('postinstall:.')])
  if (!this.args.length && npm.config.get('npat')) steps.push(
    [doOneAction, 'test', staging, this.idealTree, trackLifecycle.newGroup('npat:.')])
  if (!npm.config.get('production')) steps.push(
    [doOneAction, 'prepublish', staging, this.idealTree, trackLifecycle.newGroup('prepublish')])
