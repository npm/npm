...
install.usage = "npm install"
              ...
              + "npm install ./foo.tgz bar@stable /some/folder"
...
function install (args, cb_) {
  ...
  function cb (er, installed) {
    ...
    findPeerInvalid(where, function (er, problem) {
      ...
      var pretty = prettify(tree, installed).trim()
...
function readDependencies (context, where, opts, cb) {
  ...
  readJson(path.resolve(where, "package.json"), log.warn, function (er, data) {
    ...
    if (opts && opts.dev) {
      ...
      Object.keys(data.devDependencies || {}).forEach(function (k) {
        if (data.dependencies[k]) {
          log.warn(
            "package.json",
            "Dependency '%s' exists in both dependencies " +
    ...
    fs.readFile(wrapfile, "utf8", function (er, wrapjson) {
      ...
      log.verbose(
        "readDependencies",
        "npm-shrinkwrap.json is overriding dependencies"
      )
      var newwrap
      try {
...
function installMany (what, where, context, cb) {
  ...
  readDependencies(context, where, opt, function (er, data, wrap) {
    ...
    // resolve each one.
    asyncMap(what, targetResolver(where, context, d), function (er, targets) {
      if (er) return cb(er)
      ...
      var newAnc = Object.create(context.ancestors)
...
function targetResolver (where, context, deps) {
  ...
  function readdir(name) {
    ...
    fs.readdir(name, function (er, inst) {
      ...
      asyncMap(inst, function (pkg, cb) {
        var targetPath = path.resolve(name, pkg, "package.json")
        readJson(targetPath, log.warn, function (er, d) {
          if (er && er.code !== "ENOENT" && er.code !== "ENOTDIR") return cb(er)
  ...
  return function resolver (what, cb) {
    ...
    if (parent && parent.name === what && !npm.config.get("force")) {
      log.warn(
        "install",
        "Refusing to install %s as a dependency of itself", what
      )
      return cb(null, [])
    }
...
function localLink (target, where, context, cb) {
  ...
  var jsonFile = path.resolve(npm.globalDir, target.name, "package.json")
  var parent = context.parent
...
function resultList (target, where, parentId) {
  var nm = path.resolve(where, "node_modules")
  var targetFolder = path.resolve(nm, target.name)
  var prettyWhere = where
...
function installOne_ (target, where, context, cb_) {
  var nm = path.resolve(where, "node_modules")
  var targetFolder = path.resolve(nm, target.name)
  var prettyWhere = path.relative(process.cwd(), where)
  var parent = context.parent
  ...
  lock(nm, target.name, function (er) {
    ...
    if (targetFolder in installed) {
      log.error(
        "install",
        "trying to install", target.version,
        "to", targetFolder
      )
      log.error(
        "install",
        "but already installed versions", installed[targetFolder]
      )
...
function write (target, targetFolder, context, cb_) {
  ...
  log.silly(
    "install write",
    "writing", target.name, target.version,
    "to", targetFolder
  )
  var targetPath = path.resolve(targetFolder, "package.json")
  var contents = JSON.stringify(target, null, 2) + "\n"
  chain(
    [
      [
...
function prepareForInstallMany (packageData, depsKey, bundled, wrap, family) {
  ...
  return deps.filter(function (d) {
    ...
    var t = d + "@" + v
    log.silly(
      "prepareForInstallMany",
      "adding", t,
      "from", packageData.name,
      depsKey
    )
    return t
  })
