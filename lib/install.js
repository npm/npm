...
function readDependencies (context, where, opts, cb) {
  ...
  readJson( path.resolve(where, "package.json")
    ...
    if (er && er.code == "ENOENT") {
      return useShrinkwrap()
    }

    function useShrinkwrap () {
      var wrapfile = path.resolve(where, "npm-shrinkwrap.json")

      fs.readFile(wrapfile, "utf8", function (erNoPkg, wrapjson) {
        if (er && erNoPkg) {
          return cb(er)
        }
        if (erNoPkg) {
        ...
        if (opts && opts.dev) {
          data && Object.keys(data.devDependencies || {}).forEach(function (k) {
...
function installManyTop (what, where, context, cb_) {
  ...
  var jsonFile = path.join(where, "package.json")
  fs.exists(jsonFile, function (exists) {
    if (!exists)
      jsonFile = path.join(where, "npm-shrinkwrap.json")
