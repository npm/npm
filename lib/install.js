...
var ensureInPath = require('./bin.js').ensureInPath
...
Installer.prototype.run = function (cb) {
  ...
  chain(installSteps, function (installEr) {
    ...
    chain(postInstallSteps, function (postInstallEr) {
      ...
      ensureInPath(function isInPath (isInPathEr, isInPath) {
        if (!isInPath) {
          log.warn('PATH', 'npm bin is not in your $PATH.')
        }
        cb(installEr || postInstallEr || isInPathEr, self.getInstalledModules(), self.idealTree)
      })
    })
  })
}
