...
function installOne_ (target, where, context, cb) {
  ...
  chain
    ( [ [checkEngine, target]
      ...
      , [checkPeerDeps, target, context.family]
...
function checkPeerDeps (target, family, cb) {
  if (!target.peerDependencies || npm.config.get("force")) return cb()

  var unsatisfied = Object.keys(target.peerDependencies).map(function (d) {
    !semver.satisfies(family[d], target.peerDependencies[d])
  })
