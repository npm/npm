...
function install (where, args, cb) {
  ...
  var packageLockOnly = !!npm.config.get('package-lock-only')
  ...
  new Installer(where, dryrun, packageLockOnly, args).run(cb)
}

function Installer (where, dryrun, packageLockOnly, args) {
  validate('SBBA', arguments)
  this.where = where
  this.dryrun = dryrun
  this.packageLockOnly = packageLockOnly
  this.args = args
