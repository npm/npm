...
function save (where, installed, tree, pretty, cb) {
  if (!npm.config.get("save") &&
      !(npm.config.get("save-dev") || npm.config.get("save-optional")) ||
  ...
  fs.readFile(saveTarget, function (er, data) {
    ...
    var deps = npm.config.get("save-dev") ? "devDependencies" : "dependencies"
    deps = npm.config.get("save-optional") ? "optionalDependencies" : deps
