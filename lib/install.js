...
function installMany (what, where, context, cb) {
  ...
  readDependencies(context, where, opt, function (er, data, wrap) {
    ...
    asyncMap( what
      ...
      // Do them serially at this point.
      // Otherwise, massively parallelized unpacking can make
      // the streams cross, leading to total protonic reversal.
      chain(targets.map(function (target) { return function (cb) {
