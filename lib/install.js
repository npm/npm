...
function installOne (target, where, previously, parent, cb) {
  ...
  if (where === npm.prefix && npm.config.get("link")
    ...
    return localLink(target, where, previously, parent, cb)
  }
  installOne_(target, where, previously, parent, cb)
}

function localLink (target, where, previously, parent, cb) {
  ...
  readJson(jsonFile, function (er, data) {
    if (er || data._id === target._id) {
      if (er) {
        npm.commands.install( path.resolve(npm.config.get("prefix"), "lib")
                            , target._id
                            , function (er) {
          if (er) return cb(er, [])
          thenLink()
...
function installOne_ (target, where, previously, parent, cb) {
  var nm = path.resolve(where, "node_modules")
    , targetFolder = path.resolve(nm, target.name)
    , prettyWhere = relativize(where, process.cwd() + "/x")
