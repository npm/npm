...
var registry = require("./utils/registry")
  ...
  , isTouched = require("./utils/is-touched")
...
function installMany (what, where, previously, cb) {
  ...
  // resolve each one.
  asyncMap(what, targetResolver(where, previously), function (er, targets) {
  ...
  fs.readdir(nm, function (er, inst) {
    ...
    asyncMap(inst, function (i, cb) {
      isTouched(path.resolve(nm, i), function (er, touched) {
        log([er, touched, i], "touched?")
        if (touched) return cb(null, i)
        else return cb(null , [])
      })
    }, function (er, list) {
      log(list, "already installed")
      if (er) return cb(er)
