...
function install (tarball, name) {
  ...
  var ROOT = path.join(process.ENV.HOME, ".node_libraries"),
    ...
    targetTgz = path.join(packageRoot, name+".tgz"),
      ...
      posix.stat(tarball)
        ...
        .addCallback(function () {
          queue([
            ...
            // cd into that
            function () {
              process.cwd(packageRoot);
              var p = new process.Promise;
              setTimeout(succeed(p));
              return p;
            },
            // unpack
            unpackTar(targetTgz),
            // read the json
            readJson(path.join(packageRoot, "npm-package.json"), name),
...
function unpackTar (tarball) { return function () {
  var p = new process.Promise;
  process.createChildProcess("tar", ["xzvf", tarball, "--slice", "1"])
    .addListener("error", method(process.stdio, "writeError"))
    .addListener("output", method(process.stdio, "write"))
    .addListener("exit", function (code) {
      setTimeout(method(p, code ? "emitError" : "emitSuccess"));
    });
  return p;
}};
  ...
  posix.cat(jsonFile)
    ...
    .addCallback(function (json) {
      try {
        ...
        p.emitError("Could not parse json from " + jsonFile);
      }
    });
  return p;
}};
...
function linkLib (name, proxyFile, packageRoot) { return function () {
  ...
  posix.open(proxyFile,
    ...
    .addErrback(fail(p, "Could not open "+path.join(packageRoot, "index.js")))
    .addCallback(succeed(p));
  return p;
}};
