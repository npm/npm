...
function registryInstall (pkglist, cb) {
  ...
  // three lists: "left", "right", and "installed"
  // asyncMap over the "left" list: for each "it"
  //   find out what it is
  //   if it's version/range installed already or on "installed" list, do nothing.
  //   if it's a url, fetch to cache and add the name/version to "right"
  //   if it's a tag, fetch the json, add the version to "right"
  //   if it's a version(range) not installed, then fetch the json,
  //     add url to "right"
  //   if it's a specific version in cache, then unpack, add to "installed" list,
  //     add its deps to "right"
  // if the "right" list is not empty, then switch hands, and repeat.
  // if it is, then build all the "installed" folders.
  ...
  ;(function F (pkg) {
    if (!pkg) {
      // rollback on failure
      return buildAll(installList, function (er) {
        if (!er) return cb()
    ...
    if (!tag) {
      ...
      // unnecessarily
      var satis = (mustInstall.indexOf(pkg) === -1)
...
function getDeps (data) {
  ...
  if (Object.keys(deps).length) log(deps, "dependencies")
  return deps
}
