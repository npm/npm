...
install.usage = '\nnpm install (with no args, in package dir)' +
                ...
                '\n\nalias: npm i' +
                '\ncommon options: [--save|--save-dev|--save-optional] [--save-exact] [--with-sizes]'
...
//  a function to get the size of the files in a dir ( NOTE: recursive )
function getSizeOfDir (path) {
  var stats = fs.statSync(path)
  var total = 0

  if (!stats.isDirectory()) {
    throw new Error('It\'s not a directory! -> ' + path)
  }

  var dirList = fs.readdirSync(path)
  dirList
  .map(function (entryName) {
    var newStats = fs.statSync(path + '/' + entryName)

    if (newStats.isDirectory()) {
      return getSizeOfDir(path + '/' + entryName)
    } else {
...
Installer.prototype.printInstalled = function (cb) {
  ...
  var addedOrMoved = this.differences.filter(function (action) {
    ...
    //  add the package id
    objectToReturn.package = packageId(child)

    if (npm.config.get('with-sizes')) {
      //  get the size ( in Bytes ) of the dir of that child
      var size = getSizeOfDir(child.path)
      //  make a pretty string of the size
      var prettySize = prettyBytes(size)
      //  add the size of the package
      objectToReturn.size = prettySize
    }
