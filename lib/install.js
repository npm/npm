...
var saveAudit = require('./install/audit.js').saveAudit
...
Installer.prototype.saveToDependencies = function (cb) {
  ...
  const andAudit = (err) => {
    if (err) return cb(err)
    if (!npm.config.get('audit')) return cb()
    saveAudit(this.idealTree, this.differences, this.args, this.remove).then(()=>cb(), cb)
  }
  if (this.saveOnlyLock) {
    saveShrinkwrap(this.idealTree, andAudit)
  } else {
    saveRequested(this.idealTree, andAudit)
  }
}
