...
function Installer (where, dryrun, args) {
  ...
  this.rootPackageJsonFile = npm.config.get('package-json-file')
}
Installer.prototype = {}
...
Installer.prototype.runPreinstallTopLevelLifecycles = function (cb) {
  ...
  readPackageJson(path.join(this.where, this.rootPackageJsonFile), log, false, (err, data) => {
    if (err) return cb()
...
Installer.prototype.readGlobalPackageData = function (cb) {
  ...
  this.loadArgMetadata(iferr(cb, function () {
    mkdirp(self.where, iferr(cb, function () {
      ...
      readPackageTree(self.where, function (ctx, kid) { return ctx.parent || pkgs[kid] }, self.rootPackageJsonFile, iferr(cb, function (currentTree) {
...
Installer.prototype.readLocalPackageData = function (cb) {
  ...
  mkdirp(this.where, iferr(cb, function () {
    readPackageTree(self.where, null, self.rootPackageJsonFile, iferr(cb, function (currentTree) {
      self.currentTree = currentTree
