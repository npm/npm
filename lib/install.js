...
function install (pkglist, cb) {
  ...
  ;(function GO (pkglist) {
    ...
    asyncMap(pkglist, function (pkg, cb) {
      install_(pkg, reg, seen, mustInstall, cb)
    }, function (er, next) {
      if (er) return cb(er)
      if (next && next.length) return GO(next)
      buildAll(reg, cb)
    })
  })(pkglist)
}
...
function install_ (pkg, reg, seen, mustInstall, cb) {
  ...
  if (pkg.indexOf("/") !== -1 || pkg === ".") {
    ...
    return cache.add(pkg, finisher(pkg, reg, cb))
  }
  ...
  var name = pkg[0]
    ...
    , tag = !exact && !range && ver
    , mustHave = mustInstall === true
              || mustInstall && mustInstall.indexOf(pkg) !== -1
  ...
  if (exact) return exactInstall(pkg, name, ver, reg, cb)
  getData(name, function (er, data) {
    return er ? cb(er)
         : tag ? tagInstall(pkg, data, tag, reg, seen, mustHave, cb)
         : rangeInstall(pkg, data, range, reg, seen, mustHave, cb)
  })
}
function exactInstall (pkg, name, ver, reg, cb) {
  // if it's an exact name/version, and not already installed,
  // then just pull the data out of the cache to ensure it's there
  return cache.read(name, ver, finisher(pkg, reg, cb))
}
function tagInstall (pkg, data, tag, reg, seen, mustHave, cb) {
  var tags = data["dist-tags"]
  if (!tags[tag]) return log.er(cb, "Tag not found: "+data.name+"@"+tag)(er)
  install_(data.name+"@"+tags[tag], reg, seen, mustHave, cb)
}
function rangeInstall (pkg, data, range, reg, seen, mustHave, cb) {
  ...
  install_(data.name+"@"+satis, reg, seen, mustHave, cb)
}
...
function finisher (pkg, reg, cb) { return function (er, data) {
  ...
  cb(null, Object.keys(deps).map(function (dep) {
...
function buildAll (installed, cb) {
  ...
  asyncMap(list, function (i, cb) {
    ...
    cache.unpack(i[0], i[1], target, function (er) {
      if (er) return cb(er)
      npm.commands.build([target], cb)
    })
  }, rollbackFailure(list, cb))
}
