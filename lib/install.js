...
function install (tarball) {
  ...
  fs.stat(tarball)
    ...
    .addCallback(function () {
      ...
      ], function (f, k, lastResult) { return f(lastResult) })
        .addCallback(function () {
...
// move to ROOT/.npm/{name}/{version}/package
function moveIntoPlace (dir, pkg) { return function () {
  var p = new Promise;
  pkg = pkg.data;
  ...
  return queue([
    function () {
      ...
      path.exists(target, function (e) {
        if (e) fs.unlink(target)
          .addCallback(succeed(p))
          .addErrback(fail(p, target+" exists, and can't be removed"));
        else p.emitSuccess();
      });
      return p;
    },
...
function unpackTar (tarball, unpackTarget) { return function () {
  utils.log("unpack "+tarball);
  return processPromise("tar", ["xzvf", tarball, "--strip", "1", "-C", unpackTarget]);
}};

function readJson (jsonFile) { return function () {
  var p = new Promise;
  fs.cat(jsonFile)
...
function createMain (pkg) { return function () {
  pkg = pkg.data;
  if (!pkg.main) return;
  var p = new Promise,
    code =
...
// symlink ROOT/{name}-{version}/ to ROOT/.npm/{name}/{version}/{lib}
function linkLib (pkg) { return function () {
  pkg = pkg.data;
...
function linkMain (pkg) { return function () {
  pkg = pkg.data;
  if (!pkg.main) return;
  var
...
function runMake (pkg) { return function () {
  pkg = pkg.data;
  if (!pkg.make) return;
  var p = new Promise;
