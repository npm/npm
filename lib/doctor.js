...
var checksumCachedFiles = require('./doctor/checksum-cached-files')
...
var localNodeModules = path.join(npm.config.localPrefix, 'lib', 'node_modules')
...
function doctor (args, silent, cb) {
  ...
  var actionsToRun = [
    ...
    [checkFilesPermission, localNodeModules, 1],
    [checksumCachedFiles]
  ]
...
function makePretty (p) {
  ...
  var executableLocalModules = p[5] ? 'ok' : 'notOk'
  var checksumCachedFiles = p[6] ? 'ok' : 'notOk'
