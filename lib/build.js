...
function linkBins (pkg, folder, parent, gtop, cb) {
  ...
  var binRoot = gtop ? npm.globalBin
    ...
    , linkType = npm.config.get("bin-link-type");
                     
  log.verbose("link bins", [pkg.bin, binRoot, gtop, linkType ])
...
function linkBin (from, to, gently, type, cb) {
  
  var types = linkGetTypes(process.platform, type)
    , cbs = []
  
  if(types.shim) {
    cbs.push([ cmdShimIfExists, from, to ])
  }
  
  if(types.link) {
    cbs.push([ linkIfExists, from, to, gently ])
  }
