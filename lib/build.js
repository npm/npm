...
function autoActivate (pkg, cb) {
  if (!ini.get("auto-activate")) {
    log("auto-activate disabled by config setting", "auto-activate");
    return cb();
  }
  ...
  fs.readdir(path.join(npm.dir, pkg.name), function (er, versions) {
    ...
    if (versions.length > 1) {
      log("Not the only version, so not auto-activating", "auto-activate");
