...
function linkBins (pkg, cb) {
  ...
  steps.push(log.er(cb, "failed to link bins"))
  chain(steps)
}
...
function finishBuild (args, cb) {
  var c = args.map(function (pkg) { return function (cb) {
    chain
  ...
  c.push.apply(c, args.map(function (pkg) {
    return [ log, "Success: "+pkg._id, "build" ]
  }))
  c.push(cb)
  chain(c)
}
