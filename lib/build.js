...
function build (args, cb) {
  readAll(args, function (er, args) {
    ...
    chain(args.map(function (a) { return [lifecycle, a, "preinstall"] })
      .concat(args.map(function (a) { return [resolveDependencies, a] }))
      .concat(function (er, ok) {
          if (er) return cb(er)
          asyncMap(args, function (a, cb) {
            buildStep(a, cb)
          }, function (er) {
            if (er) return cb(er)
            chain( [ linkDependencies, args]
                 , [ finishBuild, args ]
                 , cb
                 )
          })
...
function buildStep (pkg, cb_) {
  pkg = pkg && pkg._data || pkg
  if (!pkg) return cb_(new Error("Invalid package data "+sys.inspect(pkg)))
  var i = 2
    , errState = null
  function cb (er) {
    if (errState) return
    if (er) return cb_(errState = er)
    if (-- i <= 0) return cb_(er)
  }
