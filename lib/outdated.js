...
var fetchPackageMetadata = BB.promisify(require('./fetch-package-metadata.js'))
...
function shouldUpdate (args, tree, dep, has, req, depth, pkgpath, cb, type) {
  ...
  if (tree.isLink && tree.parent && tree.parent.isTop) {
    if (parsed.type === 'directory') return skip()
    curr.version = 'linked'
  }
  ...
  BB.join(fetchPackageMetadata(parsed, tree.path), fetchPackageMetadata(npa.resolve(dep, '*'), tree.path), (wanted, latest) => {
    if (!curr || curr.version !== wanted.version || curr.version !== latest.version) {
      doIt(wanted.version, latest.version)
    } else {
      skip()
    }
  }).catch(() => {
    doIt('MISSING', 'MISSING')
  })
}
