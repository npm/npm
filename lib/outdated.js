...
function outdated (args, silent, cb) {
  ...
  outdated_(args, dir, {}, function (er, list) {
    ...
    var outTable = [ [ color.underline("Package")
                     , color.underline("Current")
                     , color.underline("Wanted")
                     , color.underline("Latest")
                     , color.underline("Location")
                   ] ].concat(outList)
...
function makePretty (p) {
  ...
  return [ ( has === want ) ? color.yellow(dep) : color.red(dep)
         ...
         , color.blackBright(parseLocation(dir))
         ]
}

function parseLocation (dir) {
  return dir.split("node_modules/").join("")
}
