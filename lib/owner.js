...
owner.completion = function (opts, cb) {
  ...
  switch (argv[2]) {
    case "ls":
      ...
      return mapNameToRegistry("/-/short", npm.config, function (er, uri) {
        if (er) return cb(er)
    ...
    case "rm":
      if (argv.length > 3) {
        ...
        return mapNameToRegistry(byUser, npm.config, function (er, uri) {
          if (er) return cb(er)
    ...
    case "add":
      ...
      return mapNameToRegistry("/-/users", npm.config, function (er, uri) {
        if (er) return cb(er)
...
function ls (pkg, cb) {
  ...
  mapNameToRegistry(pkg, npm.config, function (er, uri) {
    if (er) return cb(er)
...
function mutate (pkg, user, mutation, cb) {
  if (user) {
    ...
    mapNameToRegistry(byUser, npm.config, function (er, uri) {
      if (er) return cb(er)
  ...
  function mutate_ (er, u) {
    ...
    mapNameToRegistry(pkg, npm.config, function (er, uri) {
      ...
      registry.get(uri, null, function (er, data) {
        ...
        mapNameToRegistry(dataPath, npm.config, function (er, uri) {
          if (er) return cb(er)
