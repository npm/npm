...
var isPlainObject = require("lodash.isplainobject"); // the flat plugin map
...
var PLUGINS = [// [optionname,         plugin,                                                          default],
...
function preset(context) {
  ...
  var opts = isPlainObject(_opts) ? _opts : {}; // validate options

  var validOptions = _toConsumableArray(PLUGINS.map(function (p) {
    return p[0];
  })).concat(_toConsumableArray(Object.keys(PROXIES)));
    ...
    if (opts[proxyname] !== void 0) {
      ...
      try {
        for (var _iterator = PROXIES[proxyname][Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
          ...
          if (!pluginsMap[_to].options) {
            pluginsMap[_to].options = {};
          }

          if (!hop(pluginsMap[_to].options, proxyname)) {
            pluginsMap[_to].options[proxyname] = opts[proxyname];
          }
        }
      } catch (err) {
