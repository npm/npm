...
var userRegistries = process.env.NPM_USER_REGISTRY
if (userRegistries) {
  userRegistries = userRegistries.split(path.delimiter)
}
...
function npa (arg) {
  ...
  if (userRegistries) {
    var what = res.name || res.spec || res.rawSpec
    if (!slashRe.test(what)) {
      for(var i = 0, len = userRegistries.length, registry; i < len; i++) {
        registry = userRegistries[i]+'/'

        if (fs.existsSync(registry+what+'/package.json')) {
          parseLocal(res, registry+what)
          console.warn('use local package %s, from %s', what, registry)
          return res
        }
      }
    }
